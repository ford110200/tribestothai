...
class Lesson(db.Model):
    ...
    sound_filename_thai = db.Column(db.Text, nullable=False)  # Add sound_filename_thai column
...
def get_db():
    ...
    if db is None:
        ...
        # Optional: Enable foreign key support for SQLite
...
def student_dashboard():
    # ตรวจสอบว่ามี session ชื่อ 'username' หรือไม่ หากไม่มีให้กลับไปที่หน้า student_login
...
def transform_path(path):
    ...
    # Remove the 'static' part from the path
    if path_parts[0] == 'static':
        path_parts.pop(0)

    # Join the path parts back together to form the new path
...
def display_quiz(lesson):
    # Fetch quiz questions based on the lesson id from the Question table
    quiz_questions = Question.query.filter_by(lesson=lesson).all()
    if quiz_questions:
        for question in quiz_questions:
            question.image = transform_path(question.image)  # Transform the image path
...
def check_student_login(username, password):
    ...
    # หากพบชื่อผู้ใช้และรหัสผ่านที่ตรงกันในฐานข้อมูล จะเป็นผู้ใช้งานที่ถูกต้อง
...
def teacher_dashboard():
    ...
    # Pass the total_students variable to the template
    return render_template('teacher_dashboard.html', total_students=total_students)


@app.route('/add_user', methods=['GET', 'POST'])
def add_user():
    is_user_added = False  # เริ่มต้นตั้งค่าเป็น False
    is_username_taken = False  # เริ่มต้นตั้งค่าเป็น False
    if request.method == 'POST':
        ...
        if existing_user:
            ...
            # If the username is not found in the database, create a new user
...
def add_lesson():
    if request.method == 'POST':
        ...
        # Check if Hmong Daw sound file was uploaded
        ...
        # Check if Hmong Njua sound file was uploaded
        ...
        # Check if Lahu sound file was uploaded
        ...
        # Check if Lisu sound file was uploaded
        ...
        # Check if Arka sound file was uploaded
        ...
        return redirect(url_for('lesson_select'))  # Redirect to a relevant page after adding the lesson
...
def delete_lesson(lesson_id):
    # ลบข้อมูลที่ต้องการออกจากฐานข้อมูล
...
def add_question():
    if request.method == 'POST':
        ...
        lesson_id = int(request.form['lesson'])  # Assuming the lesson ID is submitted as an integer
